...
class InputModelList with ChangeNotifier {
  // We'll expose a stream instead of a future for real-time updates
  Stream<List<InputModel>> get inputModelStream =>
      FirestoreServices.getTransactionsStream();

  // Legacy support for future-based widgets if we don't refactor everything to StreamBuilder immediately
  // But ideally we should switch to StreamBuilder
  Future<List<InputModel>> get inputModelList =>
      FirestoreServices.getTransactionsFuture();

  void changeInputModelList() {
    // With streams, we might not need to manually notify listeners for data changes
    // if using StreamProvider or StreamBuilder, but we keep this for compatibility
    notifyListeners();
  }
}
